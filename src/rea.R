...
POFx <- function(alltags, mobiletrack, incursion, tmpshape, shape, g0param, parameters, sessiondat, type = "fast") {
  ...
  if (nrow(g0param) > 1) {
    g0Distr <- cbind(
      rpert(nRats, g0param[1, 2], g0param[1, 3], g0param[1, 4]),
    ...
    for (k in 1:nAdults) {
      if (babyRats[k] > 0) {
        if (type == "fast") {
          ...
          if (nIncurRats == 0) {
            sdm <- abs(rnorm(babyRats[k], 0, parameters[5, 3]))
          } # surveillance
          else {
            sdm <- abs(rnorm(babyRats[k], 0, parameters[6, 3]))
          } # incursion
  ...
  colnames(outPred) <- c(
    "Session", "Sens 2.5Q", "Sens 50.0Q", "Sens 97.5Q",
